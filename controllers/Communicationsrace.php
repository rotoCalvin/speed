...
class Communicationsrace
{
				...
				else
				{
					...
					else
					{
						...
						if ($playerColor == 'PINK')
						{
							...
							$response['message'] = rawurldecode($row['player_a_message']);
						...
						else if ($playerColor == 'BLUE')
						{
							...
							$response['player_time_in'] = $playerInTime;
							$response['player_time_start'] = $gameStartTime;
							$response['message'] = rawurldecode($row['player_b_message']);
							$playerTime = number_format($playerInTime - $gameStartTime, 2);
						}
						else if ($playerColor == 'ORANGE')
						{
							...
							$response['player_time_in'] = $playerInTime;
							$response['player_time_start'] = $gameStartTime;
							$response['message'] = rawurldecode($row['player_c_message']);
							$playerTime = number_format($playerInTime - $gameStartTime, 2);
						}
						else if ($playerColor == 'GREEN')
						{
							...
							$response['player_time_in'] = $playerInTime;
							$response['player_time_start'] = $gameStartTime;
							$response['message'] = rawurldecode($row['player_d_message']);
							$playerTime = number_format($playerInTime - $gameStartTime, 2);
						}
	...
	public function testStartNewGameAction()
	{
		...
		// add a new game to the games db
		$sql = "UPDATE communicationracegames SET timeout='85' WHERE game_id='".$gameId."'";
	...
	public function submitMessageAction($data)
	{
		...
		else
		{
			$gameId = strtoupper($data['game_id']);
			$playerColor = $data['color'];
			$playerMessage = rawurlencode($data['message']);
